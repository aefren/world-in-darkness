...
class World:
  ...
  def add_random_buildings(self, value, info=1):
    ...
    self.random_buildings.sort(key=lambda x: x.common, reverse=True)
    while value:
      for bu in self.random_buildings:
        ...
        if City.check_tile_req(bu, tile) and len(tile.buildings) < 1:
          ...
          bu.av_units.sort(key=lambda x: x.common, reverse=True)
          tile.buildings += [bu]
          self.buildings += [bu]
          value -= 1
          if value < 1: break
    ...
    while tries > 0 and value > 0:
      ...
      for uni in building.av_units:
        ...
        if uni.units > 1:
          ...
          uni.hp_total = randint(20, 50) * uni.hp_total / 100
          uni.update()
        uni.pos = building.pos
        uni.nation.update(uni.nation.map)
        loadsound("set10")
        building.pos.units.append(uni)
  ...
  def show_random_buildings(self, event, sound="in1"):
  ...
  def show_random_units(self, event, sound="in1"):
    sleep(loadsound(sound) / 2)
      ...
      for event in pygame.event.get():
        if event.type == pygame.KEYDOWN:
          ...
          if event.key == pygame.K_F1:
            sp.speak(f"{self.difficulty=:}, {self.random_score=:}, {self.nations_score=:}.")
          if event.key == pygame.K_i:
  ...
  def update(self, scenary):
    ...
    for t in scenary:
      ...
      for uni in t.units:
        if uni.nation in self.random_nations and uni.nation in uni.belongs: self.random_units.append(uni)
    self.nations_score = sum(n.score for n in self.nations)
    self.units = [i for i in self.units if i.hp_total >= 1]
...
class Ai:
  ...
  def ai_add_explorer(self, nation):
    ...
    logging.debug(f"{nation.score=:}, {nation.seen_threat=:}.")
    for uni in nation.units_scout:
      ...
      if (uni.pos.get_distance(uni.pos, uni.city.pos) >= distanse_limit - 1 
        ...
        uni.scout = 0
        msg = f"{uni} stops exploring."
        uni.log[-1] += [msg]
        logging.debug(msg)
      elif uni.nation.score * 0.5 < uni.nation.seen_threat:
        uni.scout = 0
    ...
    logging.debug(f"{len(nation.units_free)=:}.")
    logging.debug(f"{scout_factor=:}.")
    logging.debug(f"{scout_ranking=:}.")
    add = 0
    ...
    if event.type == pygame.KEYDOWN:
      if ctrl:
        if event.key == pygame.K_7:
          pos.add_unit(VampireLord, wallachia_t, 1)
        if event.key == pygame.K_8:
          pos.add_unit(Ghoul, wallachia_t, 1)
        if event.key == pygame.K_9:
          pos.add_unit(OrcWarrior, orcs_t, 1)
        if event.key == pygame.K_0:
          pos.add_unit(Huntress, wood_elves_t, 1)
        if event.key == pygame.K_b:
            pos.buildings += [BrigandLair(Wild, pos)]
        if event.key == pygame.K_F1:
          world.show_random_units(event)
        if  event.key == pygame.K_F2:
          world.show_random_buildings(event)
