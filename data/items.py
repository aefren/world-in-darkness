...
class Building:
    ...
    def __init__(self, nation, pos):
        ...
        self.tiles = []
        self.soil = [coast_t, glacier_t, grassland_t,
                     ocean_t, plains_t, tundra_t, waste_t]
    ...
    def improve(self, upgrade):
        ...
        if self.pos.city: msg = [
            f"se actualizará {self} en {self.pos.city}, {self.pos} {self.pos.cords}."]
    ...
    def set_hidden(self, nation, info=0):
        ...
        visible += sum([(i.units * i.sight) //
                        20 for i in self.pos.units if i.nation == nation])
        if visible < 1: return
...
class City:
    ...
    def add_building(self, itm, pos):
        ...
        if itm.pos.city:
            msg = f"se construirá {itm} en {itm.pos.city}, {itm.pos} {itm.pos.cords}."
    ...
    def add_pop(self, number, info=0):
        ...
        # avoid 0with ceil. if not add pop stops.
        grouth = ceil(number / (len(self.tiles) * 1.1))
        ...
        while number > 1:
            ...
            for t in tiles:
                ...
                if info: logging.debug(
                    f"{t}, {t.is_city= }, {len(t.buildings)= }, {_grouth= }.")
                t.last_grouth += [["turn", t.world.turn, "grouth_total",
                                   grouth, _grouth, _grouth, "number", number]]
        ...
        for bu in self.buildings:
            ...
            if bu.resource_cost[0] < bu.resource_cost[1] and bu.nation == self.nation:
                ...
                if bu.resource_cost[0] >= bu.resource_cost[1]:
                    ...
                    if bu.pos.city:
                        msg = f"{bu} {complete_t} en {bu.pos.city}, {bu.pos} {bu.pos.cords}."
                    else: msg = f". ({bu}) {complete_t} en {bu.pos} {bu.pos.cords}."
                    self.nation.log[-1].append(msg)
                    if self.nation.show_info: sleep(
                        loadsound("notify6", channel=CHT3) * 0.2)
    ...
    def check_training(self):
        ...
        if self.production:
            ...
            if self.prod_progress <= 0:
                ...
                if self.nation.show_info: sleep(
                    loadsound("notify24", channel=CHTE2) * 0.5)
                if self.production:
                    self.prod_progress = self.production[0].resource_cost
    ...
    def get_tiles_food(self, items):
        tiles = []
        buildings = [it(self.nation, self.pos)
                     for it in self.nation.av_buildings]
    ...
    def set_defense(self):
        ...
        self.defense_min /= 2
        self.defense_total_percent = round(
            self.defense_total * 100 / self.defense_min)
    ...
    def set_grouth(self):
        ...
        self.grouth_total = sum(
            [tl.grouth for tl in self.tiles]) / len(self.tiles)
    ...
    def set_seen_units(self, new=0, info=0):
        ...
        for l in self.seen_units[-7:]:
            for uni in l:
                if uni not in self.end_seen_units:
                    ...
                    if uni in self.seen_units[-7:-
                                              3]: uni.ranking -= uni.ranking * 0.4
        for uni in self.end_seen_units:
            ...
            if human_t in uni.physical_traits: self.seen_human += [uni]
            ...
            if death_t in uni.physical_traits: self.seen_undead += [uni]
        ...
        if info:
            logging.debug(
                f"fly {len(self.seen_fly)}. ranking {self.seen_fly_rnk}.")
            logging.debug(
                f"human {len(self.seen_human)}. ranking {self.seen_human_rnk}.")
            logging.debug(
                f"mounted {len(self.seen_mounted)}. ranking {self.seen_mounted_rnk}.")
            logging.debug(
                f"rng {len(self.seen_ranged)}. ranking {self.seen_ranged_rnk}.")
            logging.debug(
                f"undead {len(self.seen_undead)}. ranking {self.seen_death_rnk}.")
            logging.debug(
                f"wild {len(self.seen_wild)}. ranking {self.seen_wild_rnk}.")
    ...
    def set_train_type(self, units, info=1):
        ...
        if basics.roll_dice(1) >= 3:
            if info: logging.debug(f"sort by nation physical_traits.")
            units.sort(
                key=lambda x: any(
                    i in x.physical_traits for i in self.nation.physical_traits),
                reverse=True)
        [i.update() for i in units]
        if info: logging.debug(f"recividos {len(units)}.")
        _wild = [i for i in units if wild_t in i.physical_traits]
        ...
        _death = [i for i in units if death_t in i.physical_traits]
        if self.defense_total_percent < 100 and self.nation.defense_total_percent < 300 or self.defense_total == 0:
            ...
            if self.pos.around_threat and self.pos.around_threat >= self.defense / 2:
                units.sort(key=lambda x: x.resource_cost <=
                           self.resource_total, reverse=True)
        ...
        if self.seen_mounted_rnk > self.units_piercing_rnk + self.units_mounted_rnk:
            ...
            _units = [i for i in units if i.pn
                      or i.type == "cavalry"]
            if basics.roll_dice(1) >= 3: _units.sort(
        ...
        if self.seen_sacred_rnk * 1.5 > self.units_death_rnk:
            ...
            _units = [i for i in units if i.aligment not in [
                hell_t, malignant_t] and i.levy == 0]
            _units.sort(key=lambda x: x.ranking, reverse=True)
        ...
        if (basics.roll_dice(1) >= 5 and _anticav
            ...
            if basics.roll_dice(1) >= 3: _anticav.sort(
                key=lambda x: x.ranking, reverse=True)
        ...
        if (basics.roll_dice(1) >= 5 and _sacred
            ...
            if info: logging.debug(f"sacred.")
            if basics.roll_dice(1) >= 3: _sacred.sort(
                key=lambda x: x.ranking, reverse=True)
        ...
        if (basics.roll_dice(1) >= 5 and _archers
            ...
            if basics.roll_dice(1) >= 2: _archers.sort(
                key=lambda x: x.ranking, reverse=True)
    ...
    def status(self, info=0):
        ...
        self.buildings_food_complete = [
            i for i in self.buildings_food if i.is_complete]
        self.buildings_income = [
            i for i in self.buildings if income_t in i.tags]
        self.buildings_income_complete = [
            i for i in self.buildings_income if i.is_complete]
        self.buildings_military = [
            i for i in self.buildings if military_t in i.tags]
        self.buildings_military_complete = [
            i for i in self.buildings_military if i.is_complete]
        self.buildings_res = [i for i in self.buildings if resource_t in i.tags]
        self.buildings_res_complete = [
            i for i in self.buildings_res if i.is_complete]
        self.buildings_unrest = [
            i for i in self.buildings if unrest_t in i.tags]
        self.buildings_unrest_complete = [
        ...
        if info:
            ...
            if self.production:
                ...
                logging.debug(
                    f"entrenando {self.production[0]} en {progress} {turns_t}.")
            elif self.production == []: logging.debug(f"no está produciendo")

            logging.debug(f"amenazas internas {self.threat_inside}. ")
            logging.debug(
                f"nivel de defensa {self.defense} de {self.defense_min}, ({self.defense_percent}%).")
            ...
            logging.debug(
                f"población militar {self.pop_military}, ({round(self.military_percent, 1)}%.")
    ...
    def update(self):
        ...
        self.pop_military += sum(
            i.total_pop for i in self.nation.units if i.city == self)
        self.resource_total = sum(
            t.resource for t in self.tiles if t.blocked == 0)
        ...
        self.military_percent = round(
            self.pop_military * 100 / self.pop_total, 1)
...
class Nation:
    ...
    def __init__(self):
        ...
        self.for_res = Empty()
        self.for_res.soil = [waste_t, glacier_t,
                             grassland_t, ocean_t, plains_t, tundra_t]
    ...
    def build_food(self, city, info=0):
        ...
        for it in city.tiles:
            ...
            for bu in buildings:
                if len(
                        city.buildings_food) >= 3 and city.defense_total_percent < 250:
                    if info: logging.debug(
    ...
    def build_military(self, city, info=0):
        ...
        if self.path.build_military == "improve":
            ...
            return
        military_buildings = [
            b for b in self.av_buildings if military_t in b.tags]
        if military_buildings == []: return
        military_buildings.sort(key=lambda x: x(
            self, self.pos).resource_cost[1])
    ...
    def build_misc(self, city, info=0):
        ...
        buildings = [b for b in self.av_buildings if any(
            i in b.tags for i in [unrest_t, resource_t])]
        if buildings == []: return
        shuffle(buildings)
        ...
        for b in buildings:
            ...
            if resource_t in b.tags:
                ...
                if city.defense_total_percent < defense_min * res + 1:
                    if info: logging.debug(
                        f"needs more defense to construct {res+1} buildings")
                    continue
    ...
    def check_ppt(self, tile, info=0):
        if info: logging.info(
            f"check prefered pop tile for {tile} {tile.cords}.")
        go = 1
        log = []
    ...
    def improve_military(self, city, info=0):
        ...
        # military upgrades.
        buildings = [
            bu for bu in city.buildings_military_complete if bu.upgrade]
        if info: logging.debug(f"upgradables {len(buildings)=:}.")
        for bu in buildings:
    ...
    def set_hidden_buildings(self):
        ...
        for t in self.map:
            ...
            buildings = [b for b in buildings if self !=
                         b.nation or self not in b.nations]
    ...
    def setup_commanders(self, info=1):
        ...
        comms.sort(key=lambda x: x.pos.around_threat +
                   x.pos.threat, reverse=True)
    ...
    def status(self, info=0):
        ...
        self.upkeep_limit_percent = self.upkeep_limit_percent + \
            (self.pop / self.upkeep_change)
        if self.upkeep_limit_percent > 100: self.upkeep_limit_percent = 100
        self. upkeep_limit = round(
            self.upkeep_limit_percent * self.income / 100)
        ...
        if info:
            ...
            logging.debug(
                f"militares: {self.pop_military} ({self.military_percent}%).")
    ...
    def update(self, scenary, info=0):
        ...
        for uni in self.units:
            if animal_t in uni.physical_traits: self.units_animal.append(uni)
            if uni.can_fly: self.units_fly.append(uni)
            if human_t in uni.physical_traits: self.units_human.append(uni)
            ...
            if sacred_t in uni.physical_traits: self.units_sacred.append(uni)
            if undead_t in uni.physical_traits: self.units_undead.append(uni)
        ...
        if self.cities:
            ...
            self.defense_total = sum(
                it.defense_total for it in self.cities) / len(self.cities)
            self.defense_total_percent = sum(
                it.defense_total_percent for it in self.cities) / len(self.cities)
            self.defense_mean = int(basics.mean(
                [i.defense_total_percent for i in self.cities]))
        ...
        # expanding.
        self.expanding = 0
        if any(i for i in [ct.expanding for ct in self.cities]
               ): self.expanding = 1
...
class Unit:
    ...
    def __init__(self, nation, dead=0, units=None):
        ...
        self.effects = []
        self.favhill = [0, 1]
        self.favsoil = [coast_t, glacier_t, grassland_t,
                        plains_t, ocean_t, tundra_t, waste_t]
    ...
    def add_trait(self, trait):
        trait = trait()
        self.traits += [trait]
        exclude_names = [tr.name for tr in trait.exclude]
    ...
    def burn(self, cost=0):
        ...
        if buildings and self.mp[0] >= cost and self.can_burn:
            ...
            if building.resource_cost[0] < 1:
                ...
                if self.pos.nation and self.nation != self.pos.nation:
                    self.pos.nation.log[-1].append(msg)
                if building.nation in self.pos.world.random_nations:
                    self.pos.world.log[-1] += [msg]
                logging.debug(msg)
    ...
    def check_ready(self, info=0):
        if info: logging.info(
            f"check_ready for {self} {on_t} {self.pos} {self.pos.cords}.")
        go = 1
        for i in self.leads:
            if info: logging.debug(
                f"{i} pos {i.pos.cords}. {self} pos {self.pos.cords}.")
    ...
    def combat_dex(self, weapon):
        ...
        if weapons.index(weapon) == 0:
            self.temp_log += [
                f"att1 {self.att1+self.att1_mod} ({self.att1_mod})."]
        if weapons.index(weapon) == 1:
            self.temp_log += [
                f"att2 {self.att2+self.att2_mod} ({self.att2_mod})."]
        if weapons.index(weapon) == 2:
            self.temp_log += [
                f"att3 {self.att3+self.att3_mod} ({self.att3_mod})."]
        self.temp_log += [
    ...
    def combat_hit_to(self, target, info=0):
        ...
        if self.dist < 6:
            head_factor = 6
            size_factor = (self.size + self.size_mod) - \
                (self.target.size + self.target.size_mod)
    ...
    def combat_menu(
        ...
        if info: logging.info(
            f"hp {self} {self.hp_total}, target {target} {target.hp_total}.")
        ...
        while self.hp_total > 0 or target.hp_total > 0:
          ...
          vs {units[1]} {health_t} {units[1].hp_total}. ranking {units[1].ranking}.",
                ...
                for uni in units:
                    ...
                    [sk.run(uni)
                     for sk in uni.skills if sk.type == "before attack"]
                    if uni.dist in range(
                        uni.range[0], uni.range[1] + 1): uni.combat_settings(_round)
                    [sk.run(uni)
                     for sk in uni.skills if sk.type == "after attack"]
                ...
                [uni.combat_retreat()
                 for uni in units if uni.resolve + uni.resolve_mod >= 6]
    ...
    def combat_settings(self, _round, info=0):
        ...
        if self.hits_armor_blocked[-1]: self.temp_log += [
            f"armor blocked {self.hits_armor_blocked[-1]}."]
        if self.hits_shield_blocked[-1]: self.temp_log += [
            f"shield blocked {self.hits_shield_blocked[-1]}."]
        self.temp_log += [
    ...
    def combat_sacred_damage(self, damage, target, info=0):
        if self.damage_sacred + self.damage_sacred_mod and undead_t in target.physical_traits:
    ...
    def combat_retreat(self, info=0):
        ...
        self.update()
        if info: logging.debug(
            f"{self} retreat units {self.units} hp {self.hp_total}.")
    ...
    def create_group(self, leadership, chosen_units=None, info=1):
        ...
        if len(units) == 0:
            ...
            return
        self.log[-1] += [
            f"creates leads of {self.group_base}. on {self.pos} {self.pos.cords}."]
        leading = 0
        for i in units:
    ...
    def disband(self, hired=0):
        ...
        if Intoxicated.name in skill_names: return
        if self.hired:
    ...
    def get_cast(self):
        ...
        while True:
            ...
            if say and self.spells:
                sp.speak(
                    f"{self.spells[x].name}. {cost_t} {self.spells[x].cost}.", 1)
                say = 0
            ...
            for event in pygame.event.get():
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_F1:
                        sp.speak(
                            f"{power_t} {self.power} {of_t} {self.power_max}.")
                    if event.key == pygame.K_HOME:
                        x = 0
                        loadsound("s2")
                        say = 1
    ...
    def get_retreat_pos(self, info=0):
        ...
        if info: logging.debug(
            f"pocición inicial {self.pos} {self.pos.cords}.")
    ...
    def info(self, nation, sound="in1"):
        ...
        while True:
            ...
            if say:
                ...
                if self.defensive_skills: defensive_skills = [
                    s.name for s in self.defensive_skills]
                else: defensive_skills = "No"
                if self.global_skills: global_skills = [
                    s.name for s in self.global_skills]
                else: global_skills = "No"
                if nation in self.belongs:
                    mp = f"{self.mp[0]} {of_t} {self.mp[1]}."
                else: mp = "X"
                if self.offensive_skills: offensive_skills = [
                    s.name for s in self.offensive_skills]
                else: offensive_skills = "No"
                if self.power_max:
                    power = f"{self.power} {of_t} {self.power_max}, restores {self.power_res}."
                else: power = f"x"
                if self.shield: shield = self.shield.name
                else: shield = "no"
                if self.spells: spells = [s.name for s in self.spells]
                else: spells = "No"
                if self.terrain_skills: terrain_skills = [
                    s.name for s in self.terrain_skills]
                ...
                if self.leadership:
                    lista += [
                        f"{leadership_t} {self.leadership}.",
    ...
    def join_group(self):
        if self.leader and self.pos != self.leader.pos:
            logging.debug(
                f"{self} en {self.pos} {self.pos.cords} busca unirse a su lider {self.leader}.")
    ...
    def move_far(self):
        ...
        if sq:
            ...
            logging.debug(
                f"{self} en {self.pos} {self.pos.cords}. no hay donde mover.")
    ...
    def move_group(self, info=0):
        ...
        if info: logging.debug(
            f"{self} {self.id} at {self.pos} {self.pos.cords}..")
        ...
        threat = goto.threat
        if goto.hill and mountain_survival_t not in self.physical_traits: threat *= 1.2
        if goto.surf.name == forest_t and forest_survival_t not in self.physical_traits: threat *= 1.2
        if goto.surf.name == swamp_t and swamp_survival_t not in self.physical_traits: threat *= 1.2
        ...
        if self.forest_survival: alt.sort(
            key=lambda x: x.surf.name == forest_t, reverse=True)
        if self.swamp_survival: alt.sort(
            key=lambda x: x.surf.name == swamp_t, reverse=True)
            ...
            if ranking < threat:
                ...
                if basics.roll_dice(2) >= 10:
                    ...
                    if goto.threat <= self.ranking * 1.25:
                        if info: logging.debug(
                            f" . {goto.threat=:} {self.ranking*1,5=:}.")
                        self.move_set_(goto)
                        return
                roll = 2
    ...
    def move_unit(self, info=0):
        self.update()
        if info: logging.debug(
            f"move_unit for {self} on {self.pos}, {self.pos.cords}.")
        goto = self.goto[0][1]
        if goto == self.pos:
            self.goto = []
            self.stopped = 1
        ...
        if isinstance(self.goto[0], list):
            ...
            if info: logging.debug(msg)
            if info: logging.debug(
                f"mp {self.mp[0]} de {self.mp[1]}, costo {self.goto[0][0]}.")
            if self.leads:
                if info: logging.debug(f"lider, grupo {len(self.leads)}.")
            elif self.leader:
                if info: logging.debug(
                    f"following {self.leader} on {self.leader.pos} {self.leader.pos.cords}.")
    ...
    def oportunist_attack(self, info=1):
        ...
        for s in sq:
            ...
            if info: logging.debug(
                f"{rnd=:}, {s.threat=:}, {s.around_threat=:}")
            fear = self.get_fear()
    ...
    def raid(self, cost=0):
        if (self.pos.city and self.pos.nation != self.nation and self.can_raid
            ...
            if self.pos.raided < self.pos.income:
                logging.info(
                    f"{self} saquea a {self.pos.city} {self.pos.nation} en {self.pos} {self.pos.cords}")
            ...
            if self.pos.pop:
                ...
                defense = sum(
                    [i.units for i in self.pos.units if i.nation == self.pos.nation])
                ...
                deads = int(deads)
                self.pos.pop -= deads
                self.pos.add_corpses(
                    choice(self.pos.nation.population_type), deads)
    ...
    def random_move(self, sq=None, value=1, info=1):
        ...
        if sq:
            if self.city and self.nation in self.pos.world.random_nations: sq = [
                it for it in sq if it.get_distance(it, self.city.pos) <= self.mp[1]]
            ...
            if basics.roll_dice(1) <= self.fear:
                ...
                if basics.roll_dice(1) >= 3: sq.sort(
                    key=lambda x: x.food - x.food_need < self.food)
            ...
            if self.scout:
                ...
                if self.forest_survival: sq.sort(
                    key=lambda x: x.surf.name == forest_t, reverse=True)
                ...
                if basics.roll_dice(1) >= 6: sq.sort(
                    key=lambda x: x.hill, reverse=True)
                sq.sort(key=lambda x: x.get_distance(
                    x, self.city.pos), reverse=True)
            ...
            for s in sq:
                ...
                if rnd * fear < s.around_threat and self.fear >= 5:
                    ...
                    continue
                if info: logging.debug(
                    f"rnd {rnd} amenaza {round(s.threat)} in {s}, {s.cords}.")
                if rnd >= s.threat:
                    moved = 1
                    self.move_set(s)
                    break
    ...
    def restoring(self, info=0):
        ...
        self.terrain_skills = [
            sk for sk in self.terrain_skills if sk.turns == -1 or sk.turns > 0]
    ...
    def set_favland(self, sq):
        ...
        if self.pref_corpses: sq.sort(
            key=lambda x: len(x.corpses), reverse=True)
    ...
    def set_hidden(self, pos, info=0):
        ...
        visible += sum([it.units *
                        it.sight for it in pos.units if it.nation not in self.belongs])
    ...
    def set_ranking(self):
        ...
        if self.dfs + self.dfs_mod >= 14: self.ranking_dfs *= 2.5 + \
            (self.dfs + self.dfs_mod) / 10
        ...
        if self.res + self.res_mod >= 10: self.ranking_dfs *= 1 + \
            (self.res + self.res_mod) / 10
        ...
        if self.arm + self.arm_mod >= 8: self.ranking_dfs *= 3 + \
            (self.arm + self.arm_mod) / 10
        ...
        if self.armor:
            if self.armor.arm >= 4: self.ranking_dfs *= 1.5 + \
                (self.armor.arm / 10)
    ...
    def set_skills(self, info=0):
        ...
        tile_skills = []
        if self.pos:
            if self.attacking == 0: tile_skills += [
                sk for sk in self.pos.terrain_events + self.pos.skills + self.pos.events]
        ...
        if self.target: skills += [
            s for s in self.target.skills if s.effect == "enemy"]
    ...
    def set_tile_attr(self, info=0):
        ...
        if info and tiles: print(
            f"set_tile_attr {self} en {self.pos} {self.pos.cords}")
        for t in tiles:
    ...
    def split(self, times=1):
        ...
        for i in range(times):
            self.update()
            logging.debug(
                f"{self} hp {self.hp_total} units {self.units} mínimo {self.min_units}.")
    ...
    def unit_arrival(self, goto, info=0):
        ...
        if info: logging.debug(msg)
        self.log[-1].append(msg)
        if self.show_info and self.goto == [] and self.scout == 0: sleep(
            loadsound("walk_ft1") / 5)
        ...
        self.raid()
        self.xp += randint(0, 1)
        [b.set_hidden(self.nation)
         for b in self.pos.buildings if b.type == building_t]
        [i.set_hidden(self.pos) for i in self.pos.units
         if self.nation not in i.belongs and i.hidden]

    def unit_new_turn(self):
        logging.info(
            f"new turn {self.pos.world.turn=:} {to_t} {self} {in_t} {self.pos.cords}.")
        # init = time()
        if self.hp_total < 1: return
        self.start_turn()
    ...
    def update(self):
        ...
        self.food_total = self.food * self.units + \
            sum(i.food * i.units for i in self.leads)
        self.spells_tags = []
        ...
        ranges = []
        if self.weapon1: ranges += [self.weapon1.range_min,
                                    self.weapon1.range_max]
        if self.weapon2: ranges += [self.weapon2.range_min,
                                    self.weapon2.range_max]
        if self.weapon3: ranges += [self.weapon3.range_min,
        ...
        self.set_ranking()
        self.leads = [i for i in self.leads if i.hp_total >=
                      1 and self.nation in i.belongs]
...
class Amphibian:
    ...
    def __init__(self):
        self.soil = [coast_t, glacier_t,
                     grassland_t, plains_t, ocean_t, tundra_t]
        self.surf = [forest_t, none_t, river_t, swamp_t]
...
class DemonHunter(Elf):
    ...
    food = 3
    pop = 25
    terrain_skills = [Burn, DarkVision, DesertSurvival,
                      ForestSurvival, MountainSurvival, SwampSurvival]
...
class CursedHamlet(City):
    ...
    physical_traits = [death_t, malignant_t]
...
class Necromancer(Human):
    ...
    def levelup(self):
        if self.xp >= 10 and self.level == 1:
            self.level = 2
            self.leadership += 5
        if self.xp >= 20 and self.level == 2:
            self.level = 3
            self.leadership += 5
...
class VampireCount(Undead):
    ...
    pop = 50
    terrain_skills = [DarkVision, ForestSurvival,
                      MountainSurvival, NightSurvival]
    ...
    moves = 9
    resolve = 10
    global_skills = [DarkPresence, ElusiveShadow,
                     FearAura, Helophobia, BloodLord, ChaliceOfBlood]
...
class VladDracul(Undead):
    ...
    food = 0
    pop = 300
    terrain_skills = [DarkVision, Fly, ForestSurvival,
                      MountainSurvival, NightSurvival]
    ...
    resolve = 10
    global_skills = [DarkPresence, ElusiveShadow,
                     FearAura, Helophobia, BloodLord, MastersEye]
    ...
    def __init__(self, nation):
        super().__init__(nation)
        self.spells = [CastBloodRain, CastRainOfToads,
                       Returning, SummonBloodKnight]  # , BloodStorm
        self.favsoil = [grassland_t, plains_t, tundra_t, waste_t]
        self.favsurf = [forest_t, none_t]
        self.favhill = 0, 1
...
class Bat(Unit):
    ...
    pop = 0.5
    terrain_skills = [DarkVision, Fly, ForestSurvival,
                      MountainSurvival, SwampSurvival]
...
class Vargheist(Undead):
    ...
    moves = 9
    resolve = 10
    global_skills = [ElusiveShadow, FearAura,
                     Fly, Helophobia, NightSurvival, TheBeast]
...
class HolyEmpire(Nation):
    ...
    physical_traits = [human_t, sacred_t]
    ...
    def __init__(self):
        ...
        self.av_buildings = [Fields, PlaceOfProphecy,
                             Pastures, TrainingCamp, SawMill, Quarry]
        # Cities.
...
class WoodElves(Nation):
    ...
    def __init__(self):
        ...
        # edificios iniciales disponibles.
        self.av_buildings = [CraftmensTree, FalconRefuge,
                             GlisteningPastures, Grove, Sanctuary, StoneCarvers]
        # Cities.
        self.av_cities = [Hall]
...
class Valahia(Nation):
    ...
    physical_traits = [blood_drinker_t, death_t, vampire_t]
    ...
    def __init__(self):
        ...
        self.av_buildings = [Necropolis, Fields, HallsOfTheDeads,
                             HuntingGround, Gallows, Pit, Quarry, SawMill]
        # Cities.
...
class Witch(Human):
    ...
    def __init__(self, nation):
        ...
        self.favsoil = [grassland_t, plains_t, tundra_t]
        self.favsurf = [forest_t, none_t]
        #self.spells = [SummonBat, SummonSatir]
...
class Goblin(Unit):
    ...
    pop = 1.2
    terrain_skills = [Burn, ForestSurvival,
                      MountainSurvival, SwampSurvival, Raid]
...
class HellHound(Undead):
    ...
    food = 0
    pop = 5
    terrain_skills = [DarkVision, DesertSurvival,
                      ForestSurvival, MountainSurvival, NightSurvival]
...
class MASTER(Unit):
    ...
    unique = 1
    type = "DEBUG ONLY"
    physical_traits = [human_t, death_t, malignant_t,
                       leader_t, vampire_t, wizard_t, commander_t]
    size = 10
    gold = 0
    upkeep = 0
    resource_cost = 0
    food = 0
    pop = 0
    terrain_skills = [DesertSurvival, ForestSurvival,
                      MountainSurvival, SwampSurvival]
    ...
    moves = 12
    resolve = 10
    global_skills = [DarkPresence, ElusiveShadow, FearAura,
                     Fly, BloodLord, MastersEye, NightFerocity, NightSurvival]
    power = 2000000
    power_max = 2000000
    power_res = 5
    ...
    def __init__(self, nation):
        super().__init__(nation)
        # , BloodStorm, DarkMantle, SummonBloodKnight]
        self.spells = [CastBloodRain, RaiseDead]
        self.corpses = []
        self.favsoil = [grassland_t, plains_t, tundra_t, waste_t]
        self.favsurf = [forest_t, none_t]
        self.favhill = 0, 1
