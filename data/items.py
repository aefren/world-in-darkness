...
class City:
  ...
  def add_pop(self, number, info=0):
    ...
    while number > 1:
      ...
      for t in tiles:
        ...
        if t.is_city: _grouth *= 1.2
        if t.hill not in t.nation.pop_pref_hill: _grouth *= 0.5
        if t.soil.name not in t.nation.pop_pref_soil: _grouth *= 0.8
        if t.surf.name not in t.nation.pop_pref_surf: _grouth *= 0.6
  ...
  def population_grouth(self):
    for tl in self.tiles:
      if tl.pop == 0: tl.pop = randint(2, 6)
  ...
  def set_defense(self):
    ...
    units = [i for i in self.units if self.nation in i.belongs and i.goal == None and i.leader == None]
    ...
    for t in tiles:
      for uni in t.units: 
        if (self.nation not in uni.belongs and uni not in self.seen_units[-1] 
            and uni.hidden == 0 and uni.hp_total > 0): 
  ...
  def set_train_type(self, units, info=1):
    ...
    if self.defense_total_percent < 100 and self.nation.defense_total_percent < 300 or self.defense_total == 0:
      ...
      if basics.roll_dice(1) >= 3: units.sort(key=lambda x: x.ranged, reverse=True)
    ...
    if (_anticav and total_rnk < self.nation.units_piercing_limit):
    ...
    if (_sacred and total_rnk < self.nation.units_sacred_limit):
      if info: logging.debug(f"sacred.")
    ...
    if (_mounted and total_rnk < self.nation.units_mounted_limit):
...
class Nation:
  ...
  def build(self, city):
    ...
    if buildings:
      ...
      for b in buildings:
        ...
        for t in city.tiles:
          rate = 80
          if t.city.buildings_food == []: rate -= 20
          
          if t.city.capital: rate -= 10 
          if t.grouth <= 10: rate -= 5
          if t.grouth <= 5: rate -= 5
          if t.soil.name not in self.pop_pref_soil: rate += 10
          if t.surf.name not in self.pop_pref_surf: rate += 10
          t.food_rate = rate
          if rate <= t.populated:
            t.update(self)
    ...
    if city.buildings_food and city.buildings_military:
      ...
      for b in buildings:
        ...
        for t in city.tiles:
          t.update(self)
          if t.around_threat or t.threat > t.defense: continue
  ...
  def get_free_squads(self):
    units_free = [it for it in self.units if  it.scout == 0 and it.settler == 0
           and it.goto == [] and it.leader == None and it.goal == None
           and it.leadership == 0]
  ...
  def improve_food(self, city):
    ...
    if city.grouth >= 5: factor *= 4
    if city.grouth >= 4: factor *= 3
    if city.grouth >= 3: factor *= 2
    for b in buildings:
  ...
  def set_hidden_buildings(self, info=0):
  ...
  def setup_commanders(self, info=1):
    ...
    for com in comms:
      ...
      if com.leading < leadership: com.create_group(leadership - com.leading)
...
class Unit:
  ...
  def basic_info(self):
    sp.speak(f"{self}", 1)
    if self.leads: sp.speak(f" leads {self.leading} ({len(self.leads)}) ")
  ...
  def break_group(self):
    ...
    loadsound("set1")
    # print(f"{self} breaks leads in {self.pos} {self.pos.cords}.")
    self.group_base = 0
    self.group_score = 0
    for i in self.leads: 
      i.leader = None
  ...
  def check_position(self):
    ...
    if self.pos.is_city and self.pos.nation not in self.belongs and self.hidden == 0:
      if self.check_enemy(self.pos, is_city=1) == None: 
        if self.pos.defense >= 50 * self.pos.city.pop / 100: take_city(self)

  def check_ready(self, info=1):
    logging.info(f"check_ready for {self}.")
    go = 1
    if self.group_score > 0 or self.leads:
      if info: logging.debug(f"grupo de {self}, {self.group_score} de {self.group_base}.")
  ...
  def combat_dex(self, weapon):
    ...
    if head_damage < 1: head_damage = 0
    self.temp_log += [
    ...
    while self.hp_total > 0 or target.hp_total > 0:
      if go:
        ...
        if end_of_combat:
          ...
          """
          for i in units:
            msg = [msg1, i.battle_log]
            try:
              if i.leader == None: 
                i.log[-1][-1][1][0] += [msg, msg2]
            except: Pdb().set_trace()
            i.nation.log[-1][-1][1][0] += [msg, msg2]
            i.pos.world.log[-1][-1][1][0] += [msg, msg2]
          ...
          if self.hp_total < 1 or self.retreats:
            ...
            logging.info(msg)
            target.log[-1][-1][1][0] += [msg]
            target.nation.log[-1][-1][1][0] += [msg]
            target.pos.world.log[-1][-1][1][0] += [msg]
            self.nation.log[-1].append(msg)
            if gold_self:
              msg1 = f"gana {gold_self} {gold_t}."
              target.nation.gold += gold_self
              target.nation.log[-1][-1][1][0] += [msg1]
              logging.debug(msg1)
            if info:
              sp.speak(msg)
              sleep(loadsound("notify18") * 0.5)
            ...
            try:
              if i.leader == None:self.log[-1][-1][1][0] += [msg]
            except: Pdb().set_trace()
            self.nation.log[-1][-1][1][0] += [msg]
            target.nation.log[-1][-1][1][0] += [msg]
            target.pos.world.log[-1][-1][1][0] += [msg]
            if gold_target:
              msg1 = f"gana {gold_target} {gold_t}."
              self.nation.gold += gold_target
              self.nation.log[-1][-1][1][0] += [msg1]
              logging.debug(msg1)
            if info:
              sp.speak(msg)
  ...
  def combat_pre(self, pos, target=None):
    logging.info(f"pre_combat for {self}.")
    self.update()
    if target == None:
      _units = [it for it in pos.units if it.nation not in self.belongs and it != self
                and it.leader == None]
    ...
    squads1 = [i.name for i in leader1.squads_position]
    leader2 = defenders[0]
    if defenders[0].leader: leader2 = attackers[0].leader
    squads2 = [i.name for i in leader2.squads_position]
    ...
    msg = [msg1, [msg2]]
    for it in [self, target]:
      it.nation.log[-1] += copy.deepcopy([msg])
      it.log[-1] += copy.deepcopy([msg])
    self.pos.world.log[-1] += [copy.deepcopy(msg)]
    
    
    # Start of combat
    while attackers and defenders:
      attackers[0].combat_menu(pos, defenders[0])
  ...
  def create_group(self, score, dynamic=1, info=1):
    if score < 0: return
    logging.info(f"{self} crea grupo de {score}")
    ...
    for s in sq:
      for u in s.units:
        if u.leadership or u.goto: continue
        if u.leader or u.nation not in self.belongs: continue
        if  u.goal or u.settler: continue
        u.pos.update(self.nation) 
        if u.pos != self.pos and u.will_less: continue
        if u == self or u.pos.around_threat + u.pos.threat: continue
        if u.scout and u.pos.get_distance(u.pos, self.pos) > 2: continue
        units += [u]
    shuffle(units)
    units.sort(key=lambda x: x.pos.get_distance(self.pos, x.pos))
    
    if info: logging.debug(f"available before to set_lead_traits {len(units)}.")
    units = self.set_lead_traits(units)
    if info: logging.debug(f"available after to set_lead_traits {len(units)}.")
    if len(units) == 0:
      if info: logging.debug(f"not units available.")
      if dynamic: self.group_score = 0
      return
    self.log[-1] += [f"creates leads of {self.group_base}. on {self.pos} {self.pos.cords}."]
    for i in units:
      if info: logging.debug(f"score {self.group_score}.")
      if info: logging.debug(f"encuentra a {i}")
      if self.group_score > 0 and i not in self.leads:
        self.leads.append(i)
        i.leader = self
        i.scout = 0
        if info: logging.debug(f"{i} se une con {i.ranking} ranking.")
        self.group_score -= i.ranking
        if info: logging.debug(f"quedan {self.group_score}.")
  ...
  def get_fear(self):
    ...
    fear += self.fear/2
    return fear
  def get_hire_units(self, sound="in1"):
    sleep(loadsound(sound) / 2)
    say = 1
    x = 0
    units = [i for i in self.pos.units if self.nation not in i.belongs]
    for uni in units:
      uni._go = 1
      for t in uni.tags:
        if t not in self.lead_traits: uni._go = 0
    units = [i for i in units if i._go == 1]
    while True:
      sleep(0.01)
      if say:
        if units: 
          cost = units[x].gold * self.hire_rate
      ...
      for event in pygame.event.get():
        if event.type == pygame.KEYDOWN:
          ...
          if event.key == pygame.K_RETURN:
            if units: 
              if self.set_hire(units[x]): units.remove(units[x])
  ...
  def move_group(self, info=0):
    ...
    group = [str(i) for i in self.leads]
    if self.leads: logging.debug(f"{leads}.")
    goto = self.goto[0][1]
    goto.update(self.nation)
    
    self.pos.update(self.nation)
    ranking = sum([i.ranking for i in [self] + self.leads])
    threat = goto.threat
    ...
    if self.goal[0] == capture_t:
      if goto.threat > ranking * uniform(0.8, 1.2):
        ...
        if basics.roll_dice(1) > 4:
          ...
          self.go_home()
          return
      [i.move_set(self.goal[1]) for i in self.leads]
      goto.update(goto.nation)
      self.update()
      if self.leadership and goto.threat <= self.ranking: self.moving_unit()
      else:
        if self.leads == []:
    ...
    if self.goal[0] == stalk_t:
      if self.ranged > 5 and self.pos.day_night: ranking *= 0.2
      if ranking * 1.5 < threat:
        if info: logging.debug(f"mayor.")
        if basics.roll_dice(2) >= 10:
          if info: logging.debug(f"ataca.")
          if basics.roll_dice(1) >= 3: self.leads.sort(key=lambda x: x.ranged, reverse=True)
          for i in self.leads:
  ...
  def set_hire(self, target):
    logging.debug(f"{self} hire {self.pos.world.turn=:}.")
    cost = target.gold * self.hire_rate
    if self.nation.gold < cost:
      sp.speak(f"not enought gold", 1)
  ...
  def set_leads(self, sound="in1"):
    ...
    self.update()
    leading = self.leads
    av_units = [uni for uni in self.pos.units if uni.leadership == 0
                and uni not in self.leads]
    t = 0
    say = 1
    x = 0
    items = [leading, av_units]
    while True:
      sleep(0.01)
      if say:
        if items[t] == []: sp.speak(f"{empty_t}.")
        else: 
          sp.speak(f"{items[t][x]}")
        say = 0
      ...
      for event in pygame.event.get():
        if event.type == pygame.KEYDOWN:
          ...
          if event.key == pygame.K_RETURN:
            ...
            say = 1  
            if unit in av_units:
              sp.speak(f"{unit} added to {self}.")
              unit.leader = self
              leading += [unit]
              av_units.remove(unit)
              self.squads_position.insert(0, unit)
            elif unit in leading:
              sp.speak(f"{unit} removed from {self}.")
              unit.leader = None
              av_units += [unit]
              leading.remove(unit)
  ...
  def set_lead_traits(self, units):
    ...
    for uni in units:
      ...
      for tr in uni.traits:
        if tr not in self.lead_traits: go = 0
      if uni.aligment not in self.lead_aligments: go = 0
      if go: _units += [uni]
    return _units
  def set_lead_disband(self):
    ...
    threat = max(threats)
    if (threat < self.pos.defense*0.6
  ...
  def set_ranking(self):
    ...
    if self.strn + self.strn_mod >= 8: self.ranking_off *= 3 + (self.strn + self.strn_mod) / 10
    elif self.strn + self.strn_mod >= 6: self.ranking_off *= 2.5 + (self.strn + self.strn_mod) / 10
    ...
    if self.ranged: self.ranking += self.range[1] // 2
    for sk in self.skills:
    ...
    # if self.lead.
    if self.leads: 
      self.ranking += sum(i.ranking for i in self.leads) * 0.6
  ...
  def unit_arrival(self, goto, info=1):
  ...
  def unit_new_turn(self):
    ...
    if (self.goal and self.goal[0] == "stalking"
        ...
        or self.scout
        and self.ai == 1):
      if self.leadership == 0 and self.will_less == 0: 
        self.oportunist_attack(self)
    self.join_group()
    if self.goto: 
      self.move_unit(self)
    self.attrition()
  ...
  def update(self):
    ...
    self.squads_position = [i for i in self.squads_position if i.hp_total >= 1]
...
class SummoningCircle(HallsOfTheDead, Building):
  ...
  def __init__(self, nation, pos):
    super().__init__(nation, pos)
    self.av_units = [FellBat, Necromancer, Adjule]
    self.resource_cost = [0, 100]
    self.size = 0
...
class DarkMonolit(SummoningCircle, Building):
  ...
  def __init__(self, nation, pos):
    super().__init__(nation, pos)
    self.av_units = [BlackKnight, Isaac]
...
class BoierLord(Unit):
  ...
  def __init__(self, nation):
    ...
    self.align = Hell
    self.corpses = [Zombie]
    self.spells = [RecruitLevy, RecruitSlaveWarriors]
...
class Isaac(Unit):
  ...
  def __init__(self, nation):
    super().__init__(nation)
    self.spells = [SummonDraugr, SummonSkeletonWarriors, SummonSpectralInfantry]
    self.align = Hell
    self.corpses = [Zombie]
...
class VampireCount(Undead):
  ...
  common = 2
  lead_traits = [human_t, blood_drinker_t, bat_t, wolf_t]
...
class VarGhul(Undead):
  ...
  common = 6
  fear = 2
  lead_traits = [death_t, human_t, blood_drinker_t]
  lead_aligments = [malignant_t, hell_t, wild_t]
  populated = 1
...
class VladDracul(Undead):
  ...
  lead_traits = [human_t, death_t, vampire_t, blood_drinker_t, wolf_t]
  lead_aligments = [hell_t, malignant_t, nature_t, neutral_t, wild_t]
  pref_corpses = 1
...
class HolyEmpire(Nation):
  ...
  def __init__(self):
    ...
    # Unidades iniciales.
    self.start_units = [Decarion, PeasantLevy, PeasantLevy]
...
class WoodElves(Nation):
  ...
  def __init__(self):
    ...
    # initial units.
    self.start_units = [ForestGuard, ForestGuard, PathFinder]
...
class valahia(Nation):
  ...
  pop_pref_hill = [0]
  pop_pref_soil = [grassland_t, plains_t, tundra_t, waste_t]
  pop_pref_surf = [none_t]
  gold = 10000
  food_limit_builds = 3000
  military_limit_upgrades = 3000
  ...
  def __init__(self):
    ...
    # Unidades iniciales.
    self.start_units = [Levy, Levy, BoierLord]
...
class Campment(Building):
  ...
  def __init__(self, nation, pos):
    super().__init__(nation, pos)
    self.av_units = [Rider, Warrior, WarlockApprentice]
    self.resource_cost = [0, 40]
    self.soil = [waste_t, grassland_t, plains_t, tundra_t]
...
class FightingPit(Building):
  ...
  def __init__(self, nation, pos):
    super().__init__(nation, pos)
    self.av_units = [OrcWarrior, OrcArcher]
    self.resource_cost = [0, 50]
    self.soil = [waste_t, grassland_t, plains_t, tundra_t]
    self.surf = [forest_t, none_t]
    self.hill = [0, 1]
...
class GoblinLair(Building):
  ...
  def __init__(self, nation, pos):
    super().__init__(nation, pos)
    self.av_units = [Goblin, WolfRider]
    self.resource_cost = [0, 30]
    self.soil = [waste_t, grassland_t, plains_t, tundra_t]
    self.surf = [forest_t, swamp_t]
    self.hill = [0, 1]
...
class MammotsCave(Building):
  ...
  def __init__(self, nation, pos):
    super().__init__(nation, pos)
    self.av_units = [BlizzardWarrior, GiantOfTheLostTribe, Mammot, ShamanOfTheLostTribe]
    self.resource_cost = [0, 50]
    self.soil = [tundra_t]
    self.surf = [forest_t, none_t]
    self.hill = [0, 1]
...
class UnderworldEntrance(Building):
  ...
  def __init__(self, nation, pos):
    super().__init__(nation, pos)
    self.av_units = [Ghost, HellHound, Skeleton, WailingLady, Witch]
    self.resource_cost = [0, 80]
    self.soil = [waste_t, grassland_t, plains_t, tundra_t]
    self.surf = [forest_t, swamp_t, none_t]
    self.hill = [0, 1]
