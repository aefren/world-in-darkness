...
class Spell:
    ...
    def check_conditions(self, itm, target):
        ...
        if self.tile_pop and itm.pos.pop < self.tile_pop:
            msg += f"{self.name} {needs_t} {self.tile_pop} population."
        if msg:
...
class CastRainOfToads(Spell):
    ...
    def ai_run(self, itm):
        ...
        for t in tiles:
            ...
            if t .units: hostiles += len(
                [u.units for u in t.units if u.nation not in itm.belongs])
