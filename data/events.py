    ...
    def run(self, info=0):
        ...
        for t in self.itm.tiles:
            ...
            if roll >= 10 and t.unrest >= 20:
                ...
                buildings = [
                    b for b in t.buildings if b.nation == self.itm.nation]
...
class Revolt(Event):
    ...
    def run(self, info=1):
        ...
        for t in self.itm.tiles:
            ...
            if info: print(
                f'{t} {t.cords}. {roll = :}, {order =:}, {t.public_order =:}.')
            if roll <= order:
    ...
    def run(self, info=0):
        for t in self.itm.tiles:
            ...
            if t.populated >= 150 and basics.roll_dice(2) >= 11:
                ...
                self.itm.nation.log[-1] += [msg]
                if self.itm.nation.show_info: sleep(
                    loadsound('spell36', channel=CHTE3) // 1.3)
